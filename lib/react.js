...
function withEventsConsumer([events]) {
  ...
  return (
    <Component {...props} {...{ [events.name]: events }} />
  )
}

export function withEventsProvider(eventsComposer) {
  return withEventsProviderComponent.bind({
    eventsComposer,
  })
}

function withEventsProviderComponent(Component) {
  const { eventsComposer } = this
